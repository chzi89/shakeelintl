...
const highlights = [
  {
    ...
    desc: "Reliable logistics to 40+ countries via sea and air freight.",
  },
  {
    icon: ShieldCheck,
  ...
  return (
    <section id="export" className="py-20 sm:py-24 lg:py-32 bg-[#fffaf5]">
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-10">
        ...
        {/* Stats banner */}
        <div className="relative mx-auto mb-14 sm:mb-20 max-w-5xl `min-h-[200px] `sm:min-h-[280px] rounded-2xl sm:rounded-3xl `bg-gradient-to-br from-[#fdf6f0] to-[#f8ece0] border border-stone-200/70 shadow-sm overflow-hidden flex items-center justify-center">
          {/* Decorative globe */}
          <Globe2
            className="absolute inset-0 m-auto text-rose-300/20 pointer-events-none"
            style={{ width: "80%", height: "80%" }}
            strokeWidth={0.6}
          />
          <div className="relative z-10 grid grid-cols-2 sm:grid-cols-4 gap-6 lg:gap-10 p-6 sm:p-10 w-full">
            {stats.map((s) => (
              <div key={s.label} className="text-center">
                <div
                  className="text-3xl sm:text-4xl lg:text-5xl font-serif font-semibold"
                  style={{
                ...
                <div className="mt-2 text-xs uppercase tracking-wider text-stone-500 font-medium">
                  {s.label}
                </div>
              </div>
            ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
          {highlights.map(({ icon: Icon, title, desc }) => (
            ...
            >
              <div className="h-12 w-12 rounded-xl bg-rose-50 text-rose-500 flex items-center justify-center group-hover:bg-rose-500 group-hover:text-white transition-colors duration-300">
                <Icon width={22} height={22} />
              </div>
              <h3 className="mt-5 text-base sm:text-lg font-semibold text-stone-800">
                {title}
              </h3>
        ...
        <div className="mt-16 text-center">
          <a
            href="/contact"
            className="inline-flex items-center gap-2 rounded-full bg-rose-500 px-8 py-4 text-base font-medium text-white shadow-md hover:bg-rose-600 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200"
          >
            Request a Quote
            <svg
              viewBox="0 0 24 24"
